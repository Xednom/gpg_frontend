const blankState = {
  ...
  export const getters = {
    ...
    payment_made: (state) => state.payment_made,
  ...
  export const actions = {
    async fetchAccountBalances({ commit, dispatch }, params) {
      return await this.$axios
        .get("/api/v1/staff-account-balance/", { params: params })
        .then((res) => {
    ...
    async fetchAccountBalance({ commit, dispatch }, payload) {
      let endpoint = `/api/v1/staff-account-balance/${payload}/`;
      return await this.$axios
        .get(endpoint)
        .then((res) => {
    ...
    async saveBalance({ commit }, payload) {
      let url = "/api/v1/staff-account-balance/";
      try {
    ...
    async updatePayment({ commit }, payload) {
      let url = `/api/v1/staff-account-balance/${payload.id}/`;
      let method = "put";
