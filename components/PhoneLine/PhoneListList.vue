    ...
    <div class="row mt-5">
      <div class="col-12">
        <card card-body-classes="table-full-width">
          <h4 slot="header" class="card-title">Phone list extension list</h4>
          <div>
            <b-container fluid>
              <!-- <b-row> -->
...
export default {
  ...
  computed: {
     ...
     */
    ...mapGetters({
      phoneLines: "phoneLineExtension/phoneLines",
      phoneLine: "phoneLineExtension/phoneLine",
      pagination: "phoneLineExtension/phoneLinesPagination",
      staff: "user/staff",
      user: "user/user",
  ...
  methods: {
    ...
    async fetchPhoneLine(id) {
      await this.$store.dispatch("phoneLineExtension/fetchPhoneLine", id);
    },
    async fetchPhoneLines() {
      this.isBusy = true;
      await this.$store
        .dispatch("phoneLineExtension/fetchPhoneLines", this.pagination)
        .then(() => {
          this.isBusy = false;
        });
    },
  ...
  async mounted() {
    await this.fetchPhoneLines();
    await this.fetchMe();
    this.totalRows = this.phoneLines.length;
  },
};
</script>
<style scoped>
